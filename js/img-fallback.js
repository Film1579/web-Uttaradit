...
(function () {
  ...
  /* ---------- helper กลางสำหรับรูปของรายการ (ใช้กับสินค้า GI/OTOP) ----------
     pick(item)   : คืน path รูปของรายการ — ใช้ field "img" เป็นหลัก (เหมือน News/Travel)
                    รองรับข้อมูลเก่าที่เคยเก็บเป็น "ic" เฉพาะเมื่อค่านั้นเป็น path/URL รูปจริง (ไม่ใช่อีโมจิ)
                    ไม่มีรูป → คืน ""
     resolve(path): เติม base path ของหน้า (data-base) ให้ path สัมพัทธ์ ส่วน URL เต็ม/data: ใช้ตามเดิม
     placeholder(w,h): ภาพตัวแทนโทนเขียว–ทองแบบเดียวกับที่ใช้ตอนรูปโหลดไม่ได้ */
  var PATHLIKE = /^(assets\/|https?:\/\/|data:image\/|\/)/i;
  function pick(item) {
    if (!item) return "";
    if (item.img && PATHLIKE.test(item.img)) return item.img;
    if (item.ic && PATHLIKE.test(item.ic)) return item.ic;
    return "";
  }
  ...
  window.UTT_IMG = { pick: pick, resolve: resolve, placeholder: placeholderSrc };
