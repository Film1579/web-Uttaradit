...
window.API = (function () {
  "use strict";
  /* "json"  = อ่านจากแหล่งข้อมูลกลางจริง data/*.json ในเว็บไซต์ (ค่าเริ่มต้น — ใช้งานจริง)
     "mock"  = อ่านจาก CONFIG/DATA ที่ฝังมากับไฟล์ JS (ไว้ทดสอบ/ออฟไลน์เท่านั้น ไม่ใช่ของจริง)
     "rest"  = เผื่ออนาคตถ้าเปลี่ยนไปใช้ REST backend เต็มรูปแบบ */
  const MODE = "json";
  const BASE = "https://api.uttaradit.go.th/v1";    // ใช้เฉพาะโหมด "rest" เท่านั้น
  const TTL  = 5 * 60 * 1000;                       // cache 5 นาที
  const cache = new Map();

  const delay = ms => new Promise(r => setTimeout(r, ms));

  /* ---------- ตัวช่วยรอข้อมูลกลางจริง (data-loader.js) ก่อนอ่าน CONFIG/DATA ---------- */
  async function ready() {
    if (window.UTT_DATA_READY && typeof window.UTT_DATA_READY.then === "function") {
      await window.UTT_DATA_READY;
    }
  }
  ...
  /* ---------- JSON adapter (ของจริง) ----------
     data-loader.js เขียนทับ CONFIG.news/travel/products และ DATA.newsBody/travelBody/productBody
     ในออบเจกต์เดิม (ไม่ได้สร้างออบเจกต์ใหม่) ดังนั้น logic การกรอง/แบ่งหน้าเดียวกับ mock ใช้ซ้ำได้เลย
     สิ่งที่ต้องเพิ่มมีเพียง "รอให้โหลดจริงเสร็จก่อน" (await ready()) ในทุกเมธอด */
  const json = {};
  Object.keys(mock).forEach(key => {
    json[key] = async (...args) => { await ready(); return mock[key](...args); };
  });
  ...
  const impl = MODE === "rest" ? rest : MODE === "json" ? json : mock;
  ...
  const ui = {
    ...
    error(el, msg = window.UTT_I18N.t("common.errorLoad"), onRetry) {
      el.innerHTML = `<li class="state state--err" role="alert"><p>⚠️ ${msg}</p>
        <button class="btn btn--brand btn--sm" type="button" id="btnRetry">${window.UTT_I18N.t("common.retry")}</button></li>`;
