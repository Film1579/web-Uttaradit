...
(function () {
  ...
  function ensureModal() {
    ...
    overlay.innerHTML =
      ...
      '<button type="button" class="modal__close" aria-label="' + window.UTT_I18N.t("common.close") + '">&times;</button>' +
      '<div id="modalBody"></div></div>';
    document.body.appendChild(overlay);
    overlay.addEventListener("click", e => { if (e.target === overlay) closeModal(); });
    overlay.querySelector(".modal__close").addEventListener("click", closeModal);
    window.UTT_I18N.onChange(() => overlay.querySelector(".modal__close").setAttribute("aria-label", window.UTT_I18N.t("common.close")));
    return overlay;
  }
  function closeModal() {
    if (!overlay) return;
  ...
  document.addEventListener("click", function (e) {
    ...
    if (pending) {
      e.preventDefault();
      window.showToast(window.UTT_I18N.t("common.docPending"));
      return;
    }
