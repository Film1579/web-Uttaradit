...
(async function () {
  ...
  $("#newsQ").value = P.get("q") || "";
  const buildTabs = () => { $("#newsTabs").innerHTML = C.newsCats.map(c =>
    `<button role="tab" data-cat="${c.id}" aria-selected="${c.id===cat}" aria-controls="newsPanel">${escHtml(pick(c, "name"))}</button>`).join(""); };
  buildTabs();
  ...
  function render() {
    ...
    grid.innerHTML = list.slice((page-1)*PS, page*PS).map(n => {
      const name = escHtml(pick(C.newsCats.find(c => c.id === n.cat), "name") || "");
      return `<li class="card" data-reveal>
        <div class="card__media"><img src="../${n.img}" alt="${escHtml(pick(n, "title"))}" loading="lazy" width="600" height="375">
          <span class="card__tag">${name}</span></div>
        <div class="card__body">
          <p class="card__date"><time datetime="${n.date}">${fmtDate(n.date)}</time></p>
          <h3 class="card__title"><a href="news-detail.html?id=${n.id}">${escHtml(pick(n, "title"))}</a></h3>
          <p class="card__ex">${escHtml(pick(n, "ex"))}</p>
          <a class="card__more" href="news-detail.html?id=${n.id}">${t("common.readMore")}</a>
        </div></li>`;
    }).join("");

    pager.innerHTML = list.length <= PS ? "" :
      `<button ${page===1?"disabled":""} data-go="${page-1}" aria-label="${t("pager.prev")}">‹</button>` +
      Array.from({length: total}, (_,i) => `<button data-go="${i+1}" ${page===i+1?'aria-current="page"':""} aria-label="${t("pager.page", { n: i+1 })}">${i+1}</button>`).join("") +
  ...
  function renderSide() {
    document.querySelector("#sideLatest").innerHTML = sorted.slice(0,5).map(n =>
      `<li><a href="news-detail.html?id=${n.id}">${escHtml(pick(n, "title"))}</a><time datetime="${n.date}">${fmtDate(n.date)}</time></li>`).join("");
    document.querySelector("#sideCats").innerHTML = C.newsCats.filter(c => c.id !== "all").map(c =>
      `<li><a href="?cat=${c.id}">${escHtml(pick(c, "name"))}</a> <span style="color:var(--muted)">(${C.news.filter(n=>n.cat===c.id).length})</span></li>`).join("");
  }
  renderSide();
  render();
  I18N.onChange(() => { buildTabs(); renderSide(); render(); });   // สลับภาษา: วาดใหม่ คงหมวด/คำค้น/หน้าปัจจุบันไว้
})();
